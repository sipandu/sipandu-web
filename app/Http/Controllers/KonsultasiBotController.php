...
class KonsultasiBotController extends Controller
{
    ...
    {
        $konsultasi = Konsultasi::where('id_posyandu', auth()->guard('admin')->user()->pegawai->id_posyandu)->orderby('tanggal', 'desc')->get();
        return view('pages.admin.kesehatan-keluarga.konsultasi-bot.home', compact('konsultasi'));
    }
    ...
    {
        $pegawai = Pegawai::find(auth()->guard('admin')->user()->pegawai->id);

        $konsultasi = Konsultasi::where('kode_konsultasi', $request->kode)->first();
        $konsultasi->id_pegawai = auth()->guard('admin')->user()->pegawai->id;
        $konsultasi->nama_pemeriksa = $pegawai->nama_pegawai;
